...
| アンバー（秋） | 琥珀色 | **時間停止**：攻撃時30%で敵2ターン凍結（大:4ターン） | シトリン | 黄金色 |
| アクアマリン（冬） | アクアマリン | **クリティカル**：攻撃時15%で2倍ダメージ（大:25%） | ブルートパーズ | 空色 |
...
- **目標**: 大きな原石4種（大ローズクォーツ・大サファイア・大アンバー・大アクアマリン）をパーティで集める
- **大きな原石の配置**: 1階=大ローズクォーツ、2階=大サファイア、3階=大アンバー、4階=大アクアマリン（各フロア1個、必ずHeroから到達可能な位置に配置）
- **小さな原石**: 各フロアに2〜3個のランダムな石（本物・偽物混在）が散在
- **完了条件**: パーティメンバー（Hero or Companion）全員合わせて4種の大きな原石を保持
- **報酬**: 完了時にGold +200と勝利メッセージ
- **進捗表示**: ステータスラベルに `★/☆` で各大きな原石の入手状況を表示
- **クエスト管理**: `GemQuest` クラス（Logic.cs内）。`gemQuest` はセーブ/ロード対応。旧セーブには存在しないため try/catch でデフォルト値にフォールバック
